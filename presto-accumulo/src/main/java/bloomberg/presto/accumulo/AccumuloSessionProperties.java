...
public final class AccumuloSessionProperties
{
    ...
    private static final String INT_SECONDARY_INDEX_RANGES_PER_SPLIT = "secondar_index_ranges_per_split";
    ...
    public static final String SECONDARY_INDEX_RANGES_PER_SPLIT = "accumulo." + INT_SECONDARY_INDEX_RANGES_PER_SPLIT;
    ...
    public AccumuloSessionProperties(AccumuloConfig config)
    {
        ...
        PropertyMetadata<Boolean> s3 = booleanSessionProperty(INT_OPTIMIZE_RANGE_SPLITS_ENABLED, "Set to true to enable splitting non-indexed queries by tablets.  Default true. Disable if querying a large number of row IDs", true, false);
        PropertyMetadata<Integer> s4 = integerSessionProperty(INT_NUM_ARTIFICIAL_SPLITS, "The power-of-two for the number of artificial splits created for ranges of row IDs. A value of 0 is disabled, 1 is 2^1 splits or two splits per range, 2 is 2^2 or four splits per range, etc.  Default 0 (disabled)", 0, false);
        ...
        PropertyMetadata<Integer> s6 = integerSessionProperty(INT_SECONDARY_INDEX_RANGES_PER_SPLIT, "The number of Accumulo ranges that are packed into a single Presto split.  Default 10000", 10000, false);
        ...
        PropertyMetadata<Double> s8 = doubleSessionProperty(INT_LOWEST_CARDINALITY_THRESHOLD, "The threshold where the column with the lowest cardinality will be used instead of computing an intersection of ranges in the secondary index.  Secondary index must be enabled.  Default .01", .01, false);
    ...
    {
        return session.getProperty(INT_SECONDARY_INDEX_RANGES_PER_SPLIT, Integer.class);
    }
