...
public class UtilsTest
{
    ...
    public void testMutationIndex()
    {
        ...
        indexColumns.put(ByteBuffer.wrap(CF), ImmutableSet.of(ByteBuffer.wrap(AGE), ByteBuffer.wrap(FIRSTNAME)));
        ...
        Assert.assertArrayEquals(encode(BIGINT, 27L), indexMutations.get(0).getRow());
        Assert.assertArrayEquals(encode(VARCHAR, "alice"), indexMutations.get(1).getRow());

        Mutation m1Idx1 = new Mutation(encode(BIGINT, 27L));
        m1Idx1.put(format("%s_%s", new String(CF), new String(AGE)).getBytes(), encode(VARCHAR, "row1"), EMPTY_BYTES);
        Mutation m1Idx2 = new Mutation(encode(VARCHAR, "alice"));
        m1Idx2.put(format("%s_%s", new String(CF), new String(FIRSTNAME)).getBytes(), encode(VARCHAR, "row1"), EMPTY_BYTES);
        ...
        m2Idx1.put(format("%s_%s", new String(CF), new String(AGE)).getBytes(), encode(VARCHAR, "row2"), EMPTY_BYTES);
        Mutation m2Idx2 = new Mutation(encode(VARCHAR, "bob"));
        m2Idx2.put(format("%s_%s", new String(CF), new String(FIRSTNAME)).getBytes(), encode(VARCHAR, "row2"), EMPTY_BYTES);
