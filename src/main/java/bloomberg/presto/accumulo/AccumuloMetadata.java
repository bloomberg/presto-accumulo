        ...
        implements ConnectorMetadata
{
    ...
    @Inject
    public AccumuloMetadata(AccumuloConnectorId connectorId, AccumuloClient exampleClient)
    {
    ...
    @Override
    public AccumuloTableHandle getTableHandle(ConnectorSession session, SchemaTableName tableName)
    {
        if (!listSchemaNames(session).contains(tableName.getSchemaName())) {
            return null;
        }

        AccumuloTable table = exampleClient.getTable(tableName.getSchemaName(), tableName.getTableName());
        if (table == null) {
            return null;
        }

        return new AccumuloTableHandle(connectorId, tableName.getSchemaName(), tableName.getTableName());
    }
    ...
    {
        AccumuloTableHandle exampleTableHandle = checkType(table, AccumuloTableHandle.class, "table");
    ...
    {
        AccumuloTableHandle exampleTableHandle = checkType(tableHandle, AccumuloTableHandle.class, "tableHandle");
        checkArgument(exampleTableHandle.getConnectorId().equals(connectorId), "tableHandle is not for this connector");

        AccumuloTable table = exampleClient.getTable(exampleTableHandle.getSchemaName(), exampleTableHandle.getTableName());
        if (table == null) {
        ...
        for (ColumnMetadata column : table.getColumnsMetadata()) {
            columnHandles.put(column.getName(), new AccumuloColumnHandle(connectorId, column.getName(), column.getType(), index));
            index++;
        }
