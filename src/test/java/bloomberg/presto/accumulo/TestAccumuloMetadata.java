...
import bloomberg.presto.accumulo.AccumuloMetadata;
...
public class TestAccumuloMetadata
{
    ...
    private static final AccumuloTableHandle NUMBERS_TABLE_HANDLE = new AccumuloTableHandle(CONNECTOR_ID, "example", "numbers");
            ...
            throws Exception
    {
        ...
        AccumuloClient client = new AccumuloClient(new AccumuloConfig().setMetadata(metadataUri), CATALOG_CODEC);
        metadata = new AccumuloMetadata(new AccumuloConnectorId(CONNECTOR_ID), client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new AccumuloColumnHandle(CONNECTOR_ID, "text", VARCHAR, 0),
                "value", new AccumuloColumnHandle(CONNECTOR_ID, "value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(SESSION, new AccumuloTableHandle(CONNECTOR_ID, "unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(SESSION, new AccumuloTableHandle(CONNECTOR_ID, "example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new AccumuloTableHandle(CONNECTOR_ID, "unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new AccumuloTableHandle(CONNECTOR_ID, "example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new AccumuloTableHandle(CONNECTOR_ID, "unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new AccumuloColumnHandle(CONNECTOR_ID, "text", VARCHAR, 0)),
                new ColumnMetadata("text", VARCHAR, false));
